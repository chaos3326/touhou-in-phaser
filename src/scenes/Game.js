...
var currentRandom = 0;
function random() {
    return randomArray[currentRandom++] || (currentRandom = 1) && randomArray[0];
}
function generateNumber(value, randomRange) {
    return randomRange ? value + randomRange * random() - randomRange * 0.5 : value;
}
var globalVar = {
    time: -1,
    best: 0,
...
class Bullet extends Phaser.Physics.Arcade.Sprite {
    ...
    fire(x, y, direction, speed, sprite, danmakuData = {}) {
        ...
        for (element in defaultData) {
            !this.danmakuData[element] && (this.danmakuData[element] = defaultData[element]);
        }
        this.body.reset(x, y);
        this.setActive(true);
        ...
        this.setVelocity(speed * Math.sin(radDirection), - speed * Math.cos(radDirection));
        this.angle = direction;
        this.zise = gameConfig.hitbox[sprite.substring(sprite.indexOf('_') + 1)];
        this.setCircle(this.zise, this.width / 2 - this.zise, this.height/ 2 - this.zise);
    }
    preUpdate(time, delta) {
        ...
        (this.danmakuData.friction?.countDown) && (flag = 1) && this.handleLinearFriction();
        (this.danmakuData.spawn) && this.handleSpawn();
        this.updateAngle();
        if (flag) return;
        // if (lifetime == -1) then if (y<{} || y>{} || x<{} || x>{}) then despawn()
        (this.danmakuData.lifetime == -1) && (this.y <= -32 || this.y >=512 || this.x <=64 || this.x >= 576) && this.despawn();
        // lifetime--; if (!lifetime) then despawn()
        (this.danmakuData.lifetime == 0) && this.despawn();
        (this.danmakuData.lifetime >= 0) && !(this.danmakuData.lifetime--)  && this.despawn();
    }
    updateAngle() {
        let x = this.body.velocity.x;
        let y = this.body.velocity.y;
        let angle = (y == 0) ? (x > 0? 90 : -90) : Math.atan(-x / y) * 180 / Math.PI;
        angle += (y > 0 ? 180 : 0);
        this.setAngle(angle);
    }
    despawn() {
    ...
    handleBounce() {
        this.danmakuData.bounce.wallMargin || (this.danmakuData.bounce.wallMargin = 0);
        this.danmakuData.bounce.left && ((this.x < gameConfig.left + this.danmakuData.bounce.wallMargin) 
                    && (this.body.velocity.x < 0)) && this.bounce(true, gameConfig.left + this.danmakuData.bounce.wallMargin);
        this.danmakuData.bounce.right && ((this.x > gameConfig.right - this.danmakuData.bounce.wallMargin) 
                    && (this.body.velocity.x > 0)) && this.bounce(true, gameConfig.right - this.danmakuData.bounce.wallMargin);
        this.danmakuData.bounce.top && ((this.y < gameConfig.top + this.danmakuData.bounce.wallMargin) 
                    && (this.body.velocity.y < 0)) && this.bounce(false, gameConfig.top + this.danmakuData.bounce.wallMargin);
        this.danmakuData.bounce.bottom && ((this.y > gameConfig.bottom - this.danmakuData.bounce.wallMargin) 
                    && (this.body.velocity.y > 0)) && this.bounce(false, gameConfig.bottom - this.danmakuData.bounce.wallMargin);
        !this.danmakuData.bounce.left && ((this.x < gameConfig.left + this.danmakuData.bounce.wallMargin) 
                    && (this.body.velocity.x < 0)) && (this.danmakuData.bounce.amount = 0);
        !this.danmakuData.bounce.right && ((this.x > gameConfig.right - this.danmakuData.bounce.wallMargin) 
                    && (this.body.velocity.x > 0)) && (this.danmakuData.bounce.amount = 0);
        !this.danmakuData.bounce.top && ((this.y < gameConfig.top + this.danmakuData.bounce.wallMargin) 
                    && (this.body.velocity.y < 0)) && (this.danmakuData.bounce.amount = 0);
        !this.danmakuData.bounce.bottom && ((this.y > gameConfig.bottom - this.danmakuData.bounce.wallMargin) 
                    && (this.body.velocity.y > 0)) && (this.danmakuData.bounce.amount = 0);
        ...
        if (this.danmakuData.bounce.amount == -2) {
            (this.danmakuData.bounce.duration <= 0) && (this.danmakuData.bounce.amount = 0);
    ...
    handleGravity() {
        ...
        for (action of data.actions) {
            action.countDown && ((--action.countDown)
             || this.setAcceleration(this.body.acceleration.x + action.x, this.body.acceleration.y + action.y));
            !action.countDown && action.duration && ((--action.duration)
             || this.setAcceleration(this.body.acceleration.x - action.x, this.body.acceleration.y - action.y));
        }
    ...
    handleForce() {
        ...
        for (action of data.actions) {
            action.countDown && (--action.countDown);
            !action.countDown && action.duration && this.force(action.x, action.y, action);
        }
        (data.countDown < 0) && (data.countDown = 0)
    }
    force(x, y, action) {
...
export default class Game extends Phaser.Scene {
    init(initDanmaku) {
        ...
        initDanmaku.name && (danmakuBackup = JSON.parse(JSON.stringify(initDanmaku.danmaku)));
    ...
    preload() {
        ...
        this.load.atlas('bullets'       , 'assets/images/bullets.png', 'assets/images/bullets.json');
    ...
    create() {
        ...
        this.player.setCircle(gameConfig.hitbox.player, this.player.width /2  - gameConfig.hitbox.player
    ...
    update() {
        ...
        // set player sprite display if player go left or right, or in focus mode
        // isLeft && this.player.setFlipX(true);
        // isRight && this.player.setFlipX(false);
        ...
        if (!this.wild)
            ...
            this.handleWildDanmaku(danmaku);
    }
    addText() {
        this.hiScore = this.add.text(536, 48, this.numPadding(globalVar.best, 8), {font: "22px Arial", fill: "#FFFFFF", align: "right"})   
        this.score = this.add.text(536, 108, this.numPadding(globalVar.time, 8), {font: "22px Arial", fill: "#FFFFFF", align: "right"})
    }
    ...
    handleDanmaku(danmaku) {
        ...
        for (pattern of danmaku) {
            ...
            (pattern.countDown <= 0) && (pattern.countDown += 1 / pattern.AS) && this.shootBullet(pattern)
            if (pattern.directionSin) {
                pattern.direction = (pattern.directionCap - pattern.directionFoot) * 0.5
                * Math.sin(pattern.directionPhase / pattern.directionInterval * 6.283185)
    ...
    reloadWildDanmaku(serial) {
        this.currentWildDanmaku = JSON.parse(JSON.stringify(this.wildDanmaku[serial % this.wildDanmaku.length].danmaku));
        let pattern;
        for (pattern of this.currentWildDanmaku) {
            ...
            pattern.countDown && (pattern.countDown = (pattern.countDown - 60) * initAS / pattern.AS + 60);
            pattern.directionVV && (pattern.directionV += pattern.directionVV * serial);
        }
        this.bulletsPool.clearBullets();
        globalVar.time && this.cardget.play();
    }
    shootBullet(pattern) {
        if (typeof(pattern.sound) === 'number') {
            this.shotSound[pattern.sound].volume = (pattern.volume? pattern.volume : 0.1)
        ...
        if (pattern.objectStick) {
            ...
            originX = generateNumber(objectStick.x + pattern.positionX, pattern.randomPositionX);
            originY = generateNumber(objectStick.y + pattern.positionY, pattern.randomPositionY);
        }
        else {
        ...
        if (pattern.style == 0) {
            ...
            let direction = pattern.aim
                ? generateNumber(57.29578 * Math.atan((originX - this.player.x)/(this.player.y - originY)) 
                    + (this.player.y > originY ? 180 : 0) + pattern.direction, pattern.randomDirection)
                : generateNumber(pattern.direction, pattern.randomDirection);
            for (layer = 0; layer < pattern.layer; layer++) {
                let layerSpeed = speed + (pattern.speedLast - speed) * layer / pattern.layer;
                ...
                for (index = 0; index < pattern.amount; index++) {
                    ...
                    bulletDirection = generateNumber(bulletDirection, pattern.randomMemberDirection) + pattern.turnDirection;
                    bulletSpeed = generateNumber(layerSpeed, pattern.randomSpeed);
                    ((!pattern.failChance) || (random() > pattern.failChance)) && this.bulletsPool.fire
                            (bulletX, bulletY, bulletDirection, bulletSpeed, pattern.sprite, pattern.danmakuData);
                }
            }
        }
            ...
            for (layer = 0; layer < pattern.layer; layer++) {
                ...
                for (index = 0; index < pattern.amount; index++) {
                    bulletDirection = layerDirection + index * layerField / (pattern.amount) - layerField * 0.5 + layerField/pattern.amount * 0.5;
                    bulletX = originX + Math.cos(bulletDirection * 0.0174533);
                    bulletY = originY - Math.sin(bulletDirection * 0.0174533);
                    bulletDirection = generateNumber(bulletDirection, pattern.randomMemberDirection) + pattern.turnDirection;
                    bulletSpeed = generateNumber(layerSpeed, pattern.randomSpeed);
                    ((!pattern.failChance) || (random() > pattern.failChance)) 
                            && this.bulletsPool.fire(bulletX, bulletY, bulletDirection, bulletSpeed, pattern.sprite, pattern.danmakuData);
                }
            }
        }
    }
